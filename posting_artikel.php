	...
	<body>
		...
		<div id="wrapper">
			...
			<div id="page-content-wrapper" style="background-color:#e8ecf0; font-family: Comic Sans MS;  ">
				...
				<!-- start: Table -->
            <div class="title" align="center"><h3>Artikel</br></br></h3></div>
				<ul>
		...
		//assign a query
		$query = "SELECT file.idfile, file.nama, file.file FROM file ORDER BY idfile ";
		//execute the query
		...
		echo'<tr>';
		echo '<td><center>'.$row->idfile.'</center></td>';
		echo '<td><center>'.$row->nama.'</center></td>';
		echo '<td><center><a href="unggahfile.php?id='.$row->idfile.'">Status File </center></a></td>';
		echo '<td><center><a href="hapusdatafile.php?id='.$row->idfile.'">Hapus </center></a></td>';
		echo '</tr>';
		$i++;
}
echo '</table>';
