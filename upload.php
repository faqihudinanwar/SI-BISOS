...
<section>
	...
	<?php
		...
		if ($_FILES['userfile']['error'] > 0)
		{
			...
			echo'<br/><br/><a href = "bedahrumah.php" id="tombol">Coba Lagi</a>';
			exit;
		}
		$dir0= "uploads/";
		if(!file_exists($dir0)){
		...
		if ($upload_ok != 0){
			if (is_uploaded_file($_FILES['userfile']['tmp_name'])){
				if (move_uploaded_file($_FILES['userfile']['tmp_name'], $filename)){
					...
					//Asign a query
					$query = " UPDATE bedahrumah SET ".$tar." = '".basename($filename)."' WHERE idbedah = '".$id."'";
					// Execute the query
					...
					if (!$result){
						...
						echo'<a href = "bedahrumah.php" id="tombol">Kembali ke data bedah rumah</a>';
						$db->close();
						exit;
					}
