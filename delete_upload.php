...
<html>
	...
	//Asign a query
	$query = " UPDATE bedahrumah SET ".$tar." = '' WHERE idbedah = '".$id."'";
	// Execute the query
	...
	if (!$result){
		...
		unlink($filename);
		if(!file_exists($dir."sesudah_".$id.".jpg") && !file_exists($dir."sebelum_".$id.".jpg")){
			rmdir($dir);
		}
			echo '<h3><center> Unggah Berkas Berhasil</center></h3>';
			echo'<center><a href = "bedahrumah.php" id="tombol">Kembali ke data bedah rumah</a></center>';
		$db->close();
		exit;
	}
?>
