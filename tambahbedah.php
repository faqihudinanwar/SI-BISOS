<?php require_once("koneksi.php");
	...
	$error_nama  = $error_alamat = $error_kecamatan = $error_bantuan = "";
	$nama = $alamat = $kecamatan = $bantuan = ""; 
	...
	if (isset($_POST["submit"])){
		...
		$bantuan = test_input($_POST['bantuan']);
			if($bantuan == ''){
				$error_bantuan = "Kolom ini harus diisi";
				$valid_bantuan = FALSE;
			}elseif (!preg_match("/^[0-9]*$/",$bantuan)) {
				$error_bantuan = "Hanya angka yang diperbolehkan";
				$valid_bantuan = FALSE;
			}else {
				$valid_bantuan = TRUE;
			}	
				
		if ($valid_nama && $valid_alamat && $valid_kecamatan && $valid_bantuan ){
			...
			$bantuan   		= $koneksi->real_escape_string($bantuan);
						
			$query = "INSERT INTO bedahrumah (nama, alamat, idkec, bantuan) VALUES ('".$nama."','".$alamat."','".$kecamatan."','".$bantuan."')";
	...
	<body>
		...
		<div id="wrapper">
			...
			<table class="table table-condensed">
				...
				<tr>
					<td><label for="bantuan">Jumlah Bantuan</label></td>
					<td>
						<input name="bantuan" type="text" class="required" id="bantuan" placeholder="Jumlah Bantuan">
					</td>
					<td valign="top"><span class="error">* <?php echo $error_bantuan;?></span></td>
				</tr>
