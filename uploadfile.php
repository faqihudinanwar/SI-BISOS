...
<section>
	...
	<?php
		...
		if ($_FILES['userfile']['error'] > 0)
		{
			...
			echo'<br/><br/><a href = "posting_artikel.php" id="tombol">Coba Lagi</a>';
			exit;
		}
		$dir0= "uploads/file/";
		if(!file_exists($dir0)){
		...
		$allowed_type = array("pdf", "docx");
		if(!in_array($file_type, $allowed_type)) {
			echo "Sorry, only PDF or DOCX files are allowed.";
			$upload_ok = 0;
		}
		...
		if ($upload_ok != 0){
			if (is_uploaded_file($_FILES['userfile']['tmp_name'])){
				if (move_uploaded_file($_FILES['userfile']['tmp_name'], $filename)){
					...
					//Asign a query
					$query = " UPDATE file SET ".$tar." = '".basename($filename)."' WHERE idfile = '".$id."'";
					// Execute the query
					$result = $db->query( $query );
					if (!$result){
						die ("Could not query the database: <br />". $db->error);
					}else{
						echo '<h3><center> Unggah Berkas Berhasil</center></h3>';
						echo'<a href = "posting_artikel.php" id="tombol">Kembali ke Posting Artikel</a>';
						$db->close();
						exit;
					}
