<?php require_once("koneksi.php");
	...
	$error_desa = $error_kecamatan =  $error_nama = $error_hp  = "";
	$desa = $kecamatan = $nama = $hp = ""; 
	...
	if (isset($_POST["submit"])){
		...
		if ($desa == ''){
			$error_desa = "Bagian ini Harus diisi";
			$valid_desa = FALSE;
		}elseif (!preg_match("/^[a-zA-Z ,.():\/]*$/",$desa)) {
		   $error_desa = "Hanya Huruf, Spasi dan Simbol ,.():\ yang diperbolehkan";
		   $valid_desa = FALSE;
		}else{
			$valid_desa = TRUE;
		}
		...
		$hp = test_input($_POST['hp']);
			if($hp == ''){
				$error_hp = "Addres is required";
				$valid_hp = FALSE;
			}elseif (!preg_match("/^[0-9]*$/",$hp)) {
				$error_hp = "Hanya angka yang diperbolehkan";
				$valid_hp = FALSE;
			}else {
				$valid_hp = TRUE;
			}
		...
		if ($valid_desa && $valid_kecamatan && $valid_nama && $valid_hp ){
			$nama   		= $koneksi->real_escape_string($nama);
			$kecamatan		= $koneksi->real_escape_string($kecamatan);
			$desa 			= $koneksi->real_escape_string($desa);
			$hp   			= $koneksi->real_escape_string($hp);
						
			$query = "INSERT INTO namapkh (desa, idkec, nama, hp) VALUES ('".$desa."','".$kecamatan."','".$nama."','".$hp."')";
...
<html>
	...
	<body>
		...
		<div id="wrapper">
			...
			<div class="title"><h3>Data Nama Pendamping PKH</h3></div>
			...
			<table class="table table-condensed">
				...
				<tr>
					...
					<td>
						<input name="desa" type="text" class="required"  autofocus id="desa" placeholder="Masukkan Desa ">
					</td>
					<td valign="top"><span class="error">* <?php echo $error_desa;?></span></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<input name="hp" type="text" class="required" id="hp" placeholder="No HP">
					</td>
					<td valign="top"><span class="error">* <?php echo $error_hp;?></span></td>
				</tr>
				<tr>
					<td></td>
					<td>
