...
<section>
	...
	<?php
		...
		if ($_FILES['userfile']['error'] > 0)
		{
			...
			echo'<br/><br/><a href = "galeri.php" id="tombol">Coba Lagi</a>';
			exit;
		}
		$dir0= "uploads/galeri/";
		if(!file_exists($dir0)){
		...
		$allowed_type = array("jpg", "JPG", "jpeg","JPEG");
		...
		if ($upload_ok != 0){
			if (is_uploaded_file($_FILES['userfile']['tmp_name'])){
				if (move_uploaded_file($_FILES['userfile']['tmp_name'], $filename)){
					...
					//Asign a query
					$query = " UPDATE galeri SET ".$tar." = '".basename($filename)."' WHERE idgambar = '".$id."'";
					// Execute the query
					...
					if (!$result){
						...
						echo'<a href = "galeri.php" id="tombol">Kembali ke Galeri </a>';
						$db->close();
						exit;
					}
